# Lessons: 批量章节文档生成实战（2026-08，信号与系统第 7-10 章）

四章（抽样/通信系统/拉普拉斯/z 变换）用同一流水线批量生成 docx 的实战记录：遇到的问题、解法、与 skill 现有能力的关系。每章约一小时，全部通过验证与渲染冒烟。
...
- 高频需求：同一段落里「文字 + 粗体 + 内联 OMML」混排（如"**稳定性**：ROC 包含单位圆 `|z|=1`（充要条件）"）。
...
- 解决：`scripts/validator.py --level 2` 已覆盖 tblGrid/gridCol、cantSplit、tblHeader、shd E8EEF5、垂直居中——项目级 verify 脚本只需对齐 level 2 再加项目定制（标题文本比对、禁词列表）。
...
- 三次翻车记录（全部是 probe 设计问题，不是渲染问题）：
  - 章节没有某概念时 probe MISSING（第 8 章没有"混叠"）→ probe 用章节专属词，不要全局硬编码。
  - 文档用缩写而 probe 用全称（"收敛域" vs 文档的 "ROC"）→ probe 与文档实际用词一致。
...
固定八步：读原文 dump → 提取章节结构 → 翻译原书表格 → 写生成脚本 → 生成 → 验证 → 渲染冒烟 → 清理临时 PDF。脚本间复制改参（mapping 表 + 性质表 + 变换对表 + 系统对比表），验证通过率从第 1 章稳定到最后 1 章。
